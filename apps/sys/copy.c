 ...
 */
static int copy_one(const FileRef *src, const FileRef *dst,
                    uint8_t src_attrib, int8_t *err_vol)
{
...
static CmdErr cmd_copy(FsContext *ctx, int argc, char **argv)
{
    ...
    if (src.fs_ctx.vol_id == dst.fs_ctx.vol_id && src.fs_ctx.user_area == dst.fs_ctx.user_area && !strcasecmp(src.name, dst.name))
        return cmderr_syntax(NULL);
